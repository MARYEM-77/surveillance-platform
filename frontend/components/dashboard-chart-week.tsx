...
export function DashboardChartWeek() {
  const [data, setData] = useState([])

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("http://localhost:8000/alerts/by-weekday/")
        const json = await res.json()
        setData(json)
      } catch (error) {
        console.error("Erreur lors de la récupération des données par jour de semaine :", error)
      }
    }
